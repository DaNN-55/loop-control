...
begin
  ...
  if selected_episode.stage not in ('qc_review', 'render_ready')
    or exists (select 1 from public.review_packages package where package.episode_id = selected_episode.id and package.stage = 'qc_review' and package.invalidated_at is null and package.revision_number > selected_package.revision_number)
    or (selected_episode.stage = 'render_ready' and not exists (select 1 from public.tasks task where task.episode_id = selected_episode.id and task.task_type = 'generate_review_render' and task.status = 'failed' and task.input_snapshot #>> '{review_render,pre_render_review_package_id}' = selected_package.context_snapshot ->> 'pre_render_review_package_id')) then
