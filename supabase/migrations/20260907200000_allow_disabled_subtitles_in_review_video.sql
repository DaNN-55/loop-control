...
declare
  definition text;
  old_guard constant text := '      or coalesce(btrim(draft.subtitle_text), '''') = ''''';
  new_guard constant text := '      or (draft.subtitles_enabled and coalesce(btrim(draft.subtitle_text), '''') = '''')';
begin
  ...
  if position(old_guard in definition) = 0 or position(new_guard in definition) > 0 then
    raise exception 'generate shot review video subtitle guard has unknown or partial state';
  end if;
  ...
  if position(new_guard in definition) = 0 or position(old_guard in definition) > 0 then
    raise exception 'generate shot review video subtitle guard patch produced an invalid state';
  end if;
  execute definition;
end;
$$;
