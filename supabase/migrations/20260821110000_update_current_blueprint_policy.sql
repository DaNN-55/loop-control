...
create function public.update_current_blueprint(p_account_id uuid, p_policy jsonb)
...
begin
  ...
  if exists (select 1 from public.episodes where blueprint_version_id = updated_blueprint.id) then
    ...
    values (updated_blueprint.account_id, updated_blueprint.version, updated_blueprint.policy, false, now(), true)
...
revoke execute on function public.update_current_blueprint(uuid, jsonb) from public, anon;
grant execute on function public.update_current_blueprint(uuid, jsonb) to authenticated;

create or replace function public.require_production_asset_root()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
begin
  if new.stage = 'production_ready'::public.episode_stage and old.stage is distinct from new.stage
    and not exists (
      select 1
      from public.account_blueprint_versions blueprint
      where blueprint.id = new.blueprint_version_id
        and btrim(coalesce(blueprint.policy ->> 'asset_root', '')) <> ''
    ) then
    raise exception 'Production readiness requires an asset root' using errcode = '22023';
  end if;
  return new;
end;
$$;

create trigger episodes_require_production_asset_root
before update of stage on public.episodes
for each row execute function public.require_production_asset_root();
