...
begin
  foreach capability_key in array array['b_roll', 'soundtrack', 'narration', 'static_visual'] loop
    ...
    if capability_key = 'static_visual' then
      expected_provider := new.policy #>> '{static_visual,executor,provider}';
      expected_adapter := new.policy #>> '{static_visual,executor,adapter}';
      if (expected_provider, expected_adapter) not in (('openai', 'openai_images'), ('cloudflare', 'workers_ai_images')) then
        raise exception 'static_visual blueprint must use a registered image Adapter' using errcode = '22023';
      end if;
    else
      expected_provider := case capability_key when 'b_roll' then 'pexels' when 'soundtrack' then 'freesound' else 'google_tts' end;
      expected_adapter := case capability_key when 'b_roll' then 'pexels_video' when 'soundtrack' then 'freesound_preview' else 'google_tts' end;
    end if;
