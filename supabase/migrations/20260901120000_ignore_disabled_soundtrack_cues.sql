...
begin
  for candidate in
    select episode.id, episode.account_id, blueprint.policy as blueprint_policy, package.id as storyboard_review_package_id, package.context_snapshot as storyboard_context
  ...
  loop
    ...
    if (candidate.blueprint_policy -> 'soundtrack') is not null and jsonb_typeof(candidate.blueprint_policy -> 'soundtrack') <> 'null' and exists (
    ...
    insert into public.state_transitions (episode_id, from_stage, to_stage, reason, actor_id) values (candidate.id, 'storyboard_approved', 'production_ready', '所有已启用能力的冻结镜头与音频均已验证完成。', null);
    insert into public.audit_events (account_id, episode_id, event_type, payload, actor_id) values (candidate.account_id, candidate.id, 'stage_transition', jsonb_build_object('from_stage', 'storyboard_approved', 'to_stage', 'production_ready', 'reason', '所有已启用能力的冻结镜头与音频均已验证完成。'), null);
...
declare
  function_name text;
  definition text;
  cue_source text := 'jsonb_array_elements(coalesce(candidate.storyboard_context #> ''{worker_result,storyboard,audioCues}'', ''[]''::jsonb))';
  enabled_cue_source text := 'jsonb_array_elements(case when exists (select 1 from public.episodes episode join public.account_blueprint_versions blueprint on blueprint.id = episode.blueprint_version_id where episode.id = candidate.episode_id and (blueprint.policy -> ''soundtrack'') is not null and jsonb_typeof(blueprint.policy -> ''soundtrack'') <> ''null'') then coalesce(candidate.storyboard_context #> ''{worker_result,storyboard,audioCues}'', ''[]''::jsonb) else ''[]''::jsonb end)';
begin
  foreach function_name in array array['public.create_pre_render_review_packages()', 'public.create_pre_render_review_packages_for_episode(uuid)'] loop
    select pg_get_functiondef(to_regprocedure(function_name)) into definition;
    if definition is null or position(cue_source in definition) = 0 then
      raise exception 'Unable to gate optional soundtrack cues in %', function_name;
    end if;
    execute replace(definition, cue_source, enabled_cue_source);
  end loop;
end;
$$;
