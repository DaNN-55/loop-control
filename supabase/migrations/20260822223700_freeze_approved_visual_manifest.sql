...
begin
  for candidate in
    select episode.*, blueprint.policy, script_revision.id as script_revision_id, script_revision.storage_path as script_storage_path, script_revision.sha256 as script_sha256, script_revision.file_size as script_file_size, visual_package.id as visual_review_package_id, visual_package.revision_number as visual_review_revision_number, visual_package.task_id as visual_task_id, visual_package.artifact_id as approved_visual_artifact_id, visual_package.context_snapshot -> 'series_baseline' as frozen_series_baseline, visual_package.context_snapshot as visual_context
  ...
  loop
    ...
    visual_inputs := case
      ...
      then coalesce((select jsonb_agg(jsonb_build_object('artifactType', artifact.artifact_type, 'relativePath', artifact.relative_path, 'sha256', artifact.sha256, 'fileSize', artifact.file_size)) from public.artifacts artifact where artifact.id = candidate.approved_visual_artifact_id and artifact.artifact_type = 'visual_asset_manifest'), '[]'::jsonb)
