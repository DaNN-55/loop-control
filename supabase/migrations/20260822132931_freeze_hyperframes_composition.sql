alter table public.review_render_composition_revisions
  add column composition_config jsonb not null default '{"aspect_ratio":"9:16","width":1080,"height":1920,"captions_enabled":true,"caption_style":"cinematic","pacing":"standard","crop":"cover","transition":"fade","layout":"lower_third","narration_gain_db":0,"bgm_gain_db":-12,"sfx_gain_db":-6}'::jsonb;
...
declare candidate record; created_task public.tasks; members jsonb; inputs jsonb; composition public.review_render_composition_revisions; default_revision integer; config jsonb; project_path text; render_path text;
begin
  for candidate in
    ...
    join lateral (
      select package.* from public.review_packages package join public.approvals approval on approval.review_package_id = package.id and approval.stage = 'render_ready' and approval.decision = 'approved'
      where package.episode_id = episode.id and package.stage = 'production_ready' and package.invalidated_at is null order by package.revision_number desc limit 1
    ) package on true
...
begin
  ...
  select role into membership_role from public.account_memberships where account_id=selected_episode.account_id and user_id=auth.uid(); if membership_role is distinct from 'owner' then raise exception 'Owner membership is required to adjust a review render' using errcode='42501'; end if;
  select task.input_snapshot #> '{review_render,adjustments}' into config from public.tasks task where task.id=selected_package.task_id; if config is null then raise exception 'Frozen composition is required' using errcode='22023'; end if;
  config := config || jsonb_build_object('caption_style',p_caption_style,'pacing',p_pacing,'crop',p_crop,'transition',p_transition,'layout',p_layout);
  ...
  insert into public.review_render_composition_revisions (episode_id,pre_render_review_package_id,revision_number,caption_style,pacing,crop,transition,layout,reason,created_by,composition_config) values (selected_episode.id,(selected_package.context_snapshot ->> 'pre_render_review_package_id')::uuid,next_revision,p_caption_style,p_pacing,p_crop,p_transition,p_layout,btrim(p_reason),auth.uid(),config);
...
revoke all on function public.orchestrate_review_render_tasks(uuid), public.request_review_render_revision(uuid,text,text,text,text,text,text) from public, anon, authenticated;
