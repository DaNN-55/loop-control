...
declare candidate record; created_task public.tasks; selected_harness public.prompt_versions; allowed_tools jsonb; budget_limit integer; adapter text; executor_model text; harness_id_text text; task_snapshot jsonb;
begin
  for candidate in
    select episode.*, blueprint.policy, script_revision.id as script_revision_id, script_revision.storage_path as script_storage_path, script_revision.sha256 as script_sha256, script_revision.file_size as script_file_size, visual_package.id as visual_review_package_id, visual_package.revision_number as visual_review_revision_number, visual_package.task_id as visual_task_id, visual_package.context_snapshot -> 'series_baseline' as frozen_series_baseline
  ...
  loop
    ...
    insert into public.state_transitions (episode_id, from_stage, to_stage, reason, actor_id) values (candidate.id, 'visual_approved', 'storyboard_draft', 'Orchestrator froze the storyboard Adapter, Harness, and approved inputs.', null);
