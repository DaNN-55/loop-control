  ...
  return <article className={`shot-preparation-card${isOpen ? " is-open" : ""}${frozen ? " is-frozen" : ""}`}>
    ...
    {isOpen ? <div className="shot-preparation-body">
      ...
      <div className="shot-preparation-fields">
        ...
        {audioMode === "tts" ? <>
          <div className="shot-tts-copy-field"><div className="shot-tts-copy-header"><span>口播内容（同时作为 TTS 字幕正文）</span><button className="button button-link" disabled={frozen || isPending || isGenerating} onClick={() => { setTtsText(shot.scriptSegment); setSubtitleText(shot.scriptSegment); }} type="button"><RotateCcw aria-hidden="true" className="icon" />恢复分镜文案</button></div><textarea aria-label={`${shot.id} 口播内容`} disabled={frozen} onChange={(event) => { setTtsText(event.target.value); setSubtitleText(event.target.value); }} rows={3} value={ttsText} /></div>
