...
export function SeriesConfigurationForm({ initialName, initialRules, isEditing, isPending, onCancel, onSave }: { initialName: string; initialRules: Json; isEditing: boolean; isPending: boolean; onCancel: () => void; onSave: (name: string, rules: Json) => Promise<void> }) {
  ...
  return <form className="configuration-form series-configuration-form" onSubmit={(event) => void submit(event)}><header><div><h2>{isEditing ? `编辑 ${initialName} 的新版本` : "新建系列"}</h2><p>{isEditing ? "保存后会创建新的系列版本，已有 Episode 不会改变。" : "创建系列后，可以在新建生产单时固定系列基线。"}</p></div></header><label>系列名称<input aria-label="系列名称" onChange={(event) => setName(event.target.value)} required value={name} /></label><fieldset><legend>系列基线</legend><label>系列定位<textarea aria-label="系列定位" onChange={(event) => setForm((current) => ({ ...current, positioning: event.target.value }))} rows={2} value={form.positioning} /></label><label>内容格式<input aria-label="内容格式" onChange={(event) => setForm((current) => ({ ...current, format: event.target.value }))} value={form.format} /></label><div className="configuration-input-grid"><label>角色设定<textarea aria-label="角色设定" onChange={(event) => setForm((current) => ({ ...current, characters: event.target.value }))} rows={3} value={form.characters} /></label><label>地点设定<textarea aria-label="地点设定" onChange={(event) => setForm((current) => ({ ...current, locations: event.target.value }))} rows={3} value={form.locations} /></label><label>视觉风格<textarea aria-label="视觉风格" onChange={(event) => setForm((current) => ({ ...current, visualStyle: event.target.value }))} rows={3} value={form.visualStyle} /></label><label>叙事结构<textarea aria-label="叙事结构" onChange={(event) => setForm((current) => ({ ...current, narrativeStructure: event.target.value }))} rows={3} value={form.narrativeStructure} /></label></div><label>限制与禁用内容<textarea aria-label="限制与禁用内容" onChange={(event) => setForm((current) => ({ ...current, restrictions: event.target.value }))} rows={3} value={form.restrictions} /></label></fieldset><details className="advanced-configuration"><summary>高级系列规则（JSON）</summary><label>高级系列规则<textarea aria-label="系列规则" onChange={(event) => setForm((current) => ({ ...current, advancedJson: event.target.value }))} rows={10} value={form.advancedJson} /></label><FieldHint>这里保留 B-roll、旁白、声轨和其他特殊规则；无特殊需要时不要修改。</FieldHint></details>{error ? <p className="form-error">{error}</p> : null}<div className="configuration-actions">{isEditing ? <button className="button button-secondary" onClick={onCancel} type="button">取消编辑</button> : null}<button className="button button-primary" disabled={isPending} type="submit">{isPending ? "保存中…" : isEditing ? "创建系列新版本" : "创建系列 v1"}</button></div></form>;
}
