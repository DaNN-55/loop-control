...
import { adapterRegistration, availableExecutionPathsForCapability, isOwnerManagedConnection, localAdapterRegistrationsForCapability, mediaCapabilityForKey, registeredAdaptersForCapability } from "../worker/adapterRegistry";
...
function MediaAdapterCard({ adapterKey, connectionVersions = [], externalConnections = [], form, onChange, onCreateConnection, onRotateConnection, onTestConnection, readOnly = false, showAllowedTools = true }: { adapterKey: MediaAdapterKey; connectionVersions?: ExternalConnectionVersion[]; externalConnections?: ExternalConnection[]; form: MediaAdapterForm; onChange: (field: keyof MediaAdapterForm, value: string) => void; onCreateConnection?: (input: ExternalConnectionInput) => Promise<ExternalConnection | null>; onRotateConnection?: (input: { connectionId: string; provider: ExternalConnectionInput["provider"]; adapter: ExternalConnectionInput["adapter"]; secret: string }) => Promise<ExternalConnection | null>; onTestConnection?: (connectionId: string) => Promise<void>; readOnly?: boolean; showAllowedTools?: boolean }) {
  ...
  const availablePaths = availableExecutionPathsForCapability(definition.capability);
  ...
  const localAdapters = selectedPath === "local" ? localAdapterRegistrationsForCapability(definition.capability) : [];
  ...
  return <article className={`media-adapter-card ${statusClass}`}>
    ...
    <div className="media-adapter-field-grid">
      ...
      {selectedPath === "external" ? <>
        ...
        {ownerManagedConnection ? <ExternalConnectionPicker adapter={selectedRegisteredAdapter!.id as ExternalConnectionInput["adapter"]} connections={externalConnections} isPending={readOnly} label={label} onCreateConnection={onCreateConnection} onRotateConnection={onRotateConnection} onSelectVersion={(versionId) => onChange("credentialRef", versionId)} onTestConnection={onTestConnection} provider={selectedRegisteredAdapter!.provider as ExternalConnectionInput["provider"]} selectedVersionId={form.credentialRef} versions={connectionVersions} /> : null}
      </> : null}
      {selectedPath === "external" || selectedPath === "local" ? <>
