...
export function BlueprintConfigurationForm({ accountId, initialAssetRoot, initialPolicy, isEpisodeRepair = false, isPending, onCancel, onCreatePromptVersion, onDirtyChange, onSave, promptVersions = [], readOnly = false, systemStatus = null, technicalOnly = false }: { accountId?: string; initialAssetRoot: string; initialPolicy: Json; isEpisodeRepair?: boolean; isPending: boolean; onCancel: () => void; onCreatePromptVersion?: (input: { capability: PromptCapability; name: string; summary: string; instructions: string }) => Promise<PromptVersion | null>; onDirtyChange?: (dirty: boolean) => void; onSave: (policy: Json, activate: boolean) => Promise<void>; promptVersions?: PromptVersion[]; readOnly?: boolean; systemStatus?: LocalSystemStatusReport | null; technicalOnly?: boolean }) {
  ...
  return <section className="configuration-form blueprint-configuration-form">
    ...
    <section className="advanced-configuration" id="account-budget"><h3>分镜规划</h3>{readOnly ? <section className="stage-configuration-summary">{visibleExecutorKeys.map((executorKey) => { const executor = form.executors[executorKey]; return <article key={executorKey}><strong>{executorLabels[executorKey]}</strong><span>{executor.provider} · {executor.model}</span><span>{executor.promptVersion || "未配置 Prompt"}</span></article>; })}</section> : <StageConfigurationDialog executor={form.executors.storyboard_planning} executorKey="storyboard_planning" inline isPending={isPending} onClose={() => {}} onCreatePromptVersion={onCreatePromptVersion} onSelectPromptVersion={(version) => selectPromptVersion("storyboard_planning", version)} onUpdateExecutor={(field, value) => updateExecutor("storyboard_planning", field, value)} promptVersions={promptVersions} readOnly={readOnly} />}</section>
