...
function isConfigurableMediaAdapter(key: MediaAdapterKey): key is ConfigurableMediaAdapterKey {
  return (configurableMediaAdapterKeys as readonly MediaAdapterKey[]).includes(key);
}
...
export function BlueprintConfigurationForm({ initialAssetRoot, initialPolicy, isEpisodeRepair = false, isPending, onCancel, onCreatePromptVersion, onSave, promptVersions = [], readOnly = false }: { initialAssetRoot: string; initialPolicy: Json; isEpisodeRepair?: boolean; isPending: boolean; onCancel: () => void; onCreatePromptVersion?: (input: { capability: PromptCapability; name: string; summary: string; instructions: string }) => Promise<PromptVersion | null>; onSave: (policy: Json, activate: boolean) => Promise<void>; promptVersions?: PromptVersion[]; readOnly?: boolean }) {
  ...
  const [error, setError] = useState("");
  const [technicalConfigOpen, setTechnicalConfigOpen] = useState(readOnly);
  ...
  useEffect(() => setTechnicalConfigOpen(readOnly), [readOnly]);
  ...
  return <section className="configuration-form blueprint-configuration-form">
    <p className="blueprint-editor-note">{readOnly ? "以下按表单结构显示此蓝图版本当前保存的规则。" : isEpisodeRepair ? "只修改当前生产单需要的冻结配置；已完成工作和审核记录会保留。" : "保存会直接更新当前蓝图规则，不创建新的用户可见版本；已经创建的 Episode 仍使用自己的规则快照。"}</p>
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (adapterKey) {
        ...
        await onSave(blueprintFormToPolicy({ ...form, mediaAdapters: { ...form.mediaAdapters, [adapterKey]: repairForm } }));
        return;
      }
      if (executorKey) {
