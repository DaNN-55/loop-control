...
export function WorkerBlockerCard({ affectedTaskCount, blocker, compact = false, onOpenBlueprint }: { affectedTaskCount?: number; blocker: WorkerBlocker; compact?: boolean; onOpenBlueprint?: () => void }) {
  ...
  return <article className={`worker-blocker ${compact ? "worker-blocker-compact" : ""}`}>
    ...
    <div className={`worker-blocker-next-step ${guidance.primaryAction ? "" : "is-static"}`}>
      ...
      {guidance.primaryAction === "blueprint" && onOpenBlueprint ? <button className="button button-danger-soft button-small" onClick={onOpenBlueprint} type="button">打开蓝图配置</button> : null}
    </div>
