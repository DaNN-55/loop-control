...
export function projectHtml(taskPackage: WorkerTaskPackage): string {
  ...
  const audio = render.members.filter((member) => member.memberKind !== "shot_media").map((member, index) => `<audio id="audio-${index}" class="clip" data-start="${member.startSeconds}" data-duration="${member.durationSeconds}" data-track-index="${2 + index}" volume="${dbToGain(member.memberKind === "narration" ? adjustments.narrationGainDb : member.audioKind === "sfx" ? adjustments.sfxGainDb : adjustments.bgmGainDb)}" preload="auto" src="${mediaFor(member.relativePath)}"></audio>`).join("\n");
  return `<!doctype html>
